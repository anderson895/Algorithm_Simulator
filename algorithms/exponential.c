...
int get_quantum(int queue) {
    return 2 * (queue + 1) * (queue + 1); // 2 * k^2
}

void exponential_feedback(Process processes[], int n) {
        ...
        for (int q = 0; q < QUEUES; q++) {
            for (int i = 0; i < n; i++) {
                ...
                if (p->remaining_time > 0 && p->arrival_time <= time && p->queue == q) {
                    int quantum = get_quantum(p->queue);
...
int main() {
    ...
    exponential_feedback(processes, n);
    free(processes);
